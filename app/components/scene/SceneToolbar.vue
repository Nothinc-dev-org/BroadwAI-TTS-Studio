...
const titleDraft = ref(props.scene.title);
...
async function playAudioBytes(bytes: number[]) {
    revokeSelectionBlob();
    selectionObjectUrl = URL.createObjectURL(
    ...
    await new Promise<void>((resolve, reject) => {
        el.onended = () => resolve();
        el.onerror = () =>
            reject(new Error("No se pudo reproducir un bloque seleccionado"));
...
function startTitleEdit() {
    titleDraft.value = props.scene.title;
...
async function confirmTitleEdit() {
    ...
    if (!title) {
        error.value = "El título de la escena no puede estar vacío.";
        return;
    }
    ...
    error.value = null;
    try {
        await scenes.update({ id: props.scene.id, title });
...
<template>
    <div class="space-y-2">
        ...
        >
            <div class="min-w-0">
                <div class="flex min-w-0 items-center gap-1">
                    ...
                    <UButton
                        ...
                        variant="ghost"
                        :icon="
                            editingTitle ? 'i-lucide-check' : 'i-lucide-pencil'
                        "
                        :loading="savingTitle"
                        :aria-label="
                            editingTitle ? 'Confirmar título' : 'Editar título'
                        "
                        @click="
                            editingTitle ? confirmTitleEdit() : startTitleEdit()
                        "
                    />
                </div>
                <p v-if="scene.description" class="truncate text-sm text-muted">
                    {{ scene.description }}
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                ...
                <UButton
                    ...
                    :disabled="
                        generating || playing || playingSelection || exporting
                    "
                    @click="emit('delete')"
                >
                </UButton>
        ...
        <div
            v-if="mixSrc"
            class="flex items-center gap-2 rounded-lg border border-default px-4 py-2"
        >
            <UButton
                size="xs"
                variant="soft"
